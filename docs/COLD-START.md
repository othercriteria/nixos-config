...
target so `make reveal-secrets` can find a private key.

On a trusted host that already has the key:
...
scp ~/gnupg-{public,secret}.asc ~/gnupg-ownertrust.txt <target>:~/
```

On the target (interactive ssh — pinentry-curses needs a real TTY):

```sh
gpg --import ~/gnupg-public.asc
gpg --import ~/gnupg-secret.asc           # prompts for passphrase
gpg --import-ownertrust ~/gnupg-ownertrust.txt
gpg --list-secret-keys                    # verify
rm ~/gnupg-{public,secret}.asc ~/gnupg-ownertrust.txt
```
...
directory names. The backing service is a small FastAPI app on
`127.0.0.1:8765` inside a strict systemd sandbox
(`modules/hardened-service.nix`), fronted by nginx with HTTP Basic Auth
...
1. **Smoke-test the deploy.** With `custom.trivia.seedFixtures = true`
   ...
   curl -i -u trivia:RIGHT https://trivia.valueof.info/                # 200, HTML index
   curl -i -u trivia:RIGHT https://trivia.valueof.info/warmup/         # 200
   curl -i -u trivia:RIGHT https://trivia.valueof.info/warmup/welcome.pdf  # 200
   curl -i -u trivia:RIGHT https://trivia.valueof.info/round-late/     # 404 (upcoming)
   curl -i -u trivia:RIGHT https://trivia.valueof.info/has-escape/escape.txt # 404 (symlink)
   curl -i -u trivia:RIGHT https://trivia.valueof.info/dir-is-symlink/ # 404 (root-escape)
   curl -i -u trivia:RIGHT https://trivia.valueof.info/has-dotfile/.secret # 404 (dotfile)
   ```
